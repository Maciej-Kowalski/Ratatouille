...
class FusionMouseTiming:
    ...
    def start(self):
        ...
        # Start output threads
        IMU_output_thread = threading.Thread(target=self.IMU_output, args=(self.IMU_processed_data_queue, self.stop_event, FUSION_BUFFER_SIZE))
        self.threads.append(IMU_output_thread)
    ...
    def unpack(self, data):
        if len(data) == FUSION_PACKET_LENGTH-1:
            audio = np.frombuffer(data[0:AUDIO_OFFSET], dtype=np.int16)
            self.audio_raw_data_queue.put(audio)
            timestamp = struct.unpack('<I', data[AUDIO_OFFSET:AUDIO_OFFSET+4])[0]
            self.IMU_raw_data_queue.put(timestamp)
        else:
            print("Data packet length invalid - ", len(data), ", expected ", FUSION_PACKET_LENGTH-1)
            print(data)
