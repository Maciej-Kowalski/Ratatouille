...
# Cannot find submodule core, hence copied below:
...
class OREKF:
    ...
    def __init__(self,
        ...
        ##### User created #####
        self.sig: np.ndarray = 0
        self.eps: np.ndarray = 0
    ...
    def rupdate(self, q: np.ndarray, gyr: np.ndarray, acc: np.ndarray, mag: np.ndarray = None, dt: float = None) -> np.ndarray:
        """
        ...
        #Q_t = self.g_noise                      # Updated Process Noise Covariance
        ...
        #print(f"v1: {v[0]:8.4f}, v2: {v[1]:8.4f}, v3: {v[2]:8.4f}")
        ...
        ##### EKF MD str ##### https://ieeexplore.ieee.org/document/10472882
        # sat1  = ct.saturation_nonlinearity(0.15,-1)  # Standard scalar symmetric saturation function
        # vs = sat1(v)
        # zeta = 10
        # beta = v.T@np.linalg.inv(S)@v
        # #print(beta)
        # # P(beta <= MD^2) = 1 - alpha2
        # #P_t = ld*(F@self.P@F.T + Q_t) # Replaces P_t = F@self.P@F.T + Q_t

        # MDw = 1/(1+math.exp(-beta + zeta))
        # self.R = 1*MDw*MDw
        # #print(self.R)

        ##### S   = H@P_t@H.T + Rd                # Updated Measurement Prediction Covariance, where Rd = np.array([[4*MDw[i]]]) is dynamically adjusted (was self.R)
        MD = []
        MD.append(math.sqrt((v[0]*v[0])/S[0,0]))             # Mahalanobis Distance (MD) https://medium.com/blogyuxiglobal/kalman-filter-the-way-to-remove-outliers-bb6aa616788e, https://ieeexplore.ieee.org/document/8453333
        MD.append(math.sqrt((v[1]*v[1])/S[1,1]))
        ...
        #print(MDw)
        Rnew = np.diag([4*MDw[0], 4*MDw[1], 4*MDw[2]])
        self.R = Rnew
        #print(Rnew)
        #self.noises = [np.float64(1*MDw[0]), np.float64(1*MDw[1]), np.float64(1*MDw[2])]
        #print(self.noises)
        #self.R = [(MD[0]*MD[0]),(MD[1]*MD[1]),(MD[2]*MD[2])]
        #self.R = Rnew # NEED TO MAKE THIS DIAGONAL
        # self.R = Rnew
        # np.diag(self.noises[1], 3) = Rnew
        S   = H@P_t@H.T + Rnew
        #print(self.R)
        ...
        # epsd = lb2*eps + gam2*v*v
        # sat = ct.saturation_nonlinearity(0.15,-0.15)
        # vs = sat(v)
        ...
        #     print("sat")
            #self.q = q_t + 1*(K@v)
        #self.R = [0.05*MDw[0],0.05*MDw[1],0.05*MDw[2]]
        # if( (v[0] > 0.2) or (v[1] > 0.2) or (v[2] > 0.2) ):
        #     self.q = q + K@vs                       # Corrected State with standard scalar symmetric saturation function
        #     print("sat")
        # else:
        #     self.q = q_t + K@vs                       # Corrected State with standard scalar symmetric saturation function

        # if( (MD[0]*MD[0]) > zeta or (MD[1]*MD[1] > zeta) or (MD[2]*MD[2] > zeta) ):
        #     # self.q = (0.9*q) + 0.1*(q_t + K@v)
        #     self.q = q + 0.2*q_t
        #     print("err")
        #     #print(self.R)
        #     #self.R = [x * 4 for x in self.R]
        #     self.R = [x * 4 for x in self.R]
        # else:
        #     self.q = q_t + K@v
