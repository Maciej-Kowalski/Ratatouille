...
void print_buffer_as_dec(uint8_t *buffer, size_t length);
/*unused/legacy */
...
/*void print_buffer(uint8_t *buffer, size_t length) {
	for (uint8_t i = 0; i < length; i++) {
	    // Use snprintf to format each value and append to msg
	    int len = snprintf(msg + msg_len, sizeof(msg) - msg_len, "a[%d]: %d, ", i, a_int[i]);
	    msg_len += len;  // Update the total message length
