...
data = csv.reader(open('9April-Drift.csv', 'rt'), delimiter=',')
...
for row in data:
    ...
    rr.append(round(float(row[6]),3))
    rp.append(round(-float(row[7]),3))
    ry.append(round(-float(row[8]),3))


plt.plot(list(np.arange(len(ry))*0.01), ry, label='EKF Yaw')
plt.plot(list(np.arange(len(y))*0.01), y, label='REKF Yaw')
plt.plot(list(np.arange(len(fy))*0.01), fy, label='Filtered Yaw')
ax = plt.gca()
